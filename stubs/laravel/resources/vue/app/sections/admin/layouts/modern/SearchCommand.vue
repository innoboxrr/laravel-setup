<template>
    <TransitionRoot :show="open" as="template" @after-leave="closeCommand" appear>
        <Dialog as="div" class="relative" @close="open = false" style="z-index: 999">
            <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0" enter-to="opacity-100" leave="ease-in duration-200" leave-from="opacity-100" leave-to="opacity-0">
                <div class="fixed inset-0 bg-slate-500 opacity-80 transition-opacity" />
            </TransitionChild>
...
const people = [
    ...
    { id: 2, name: 'Leslie Hom', url: '#' },
    { id: 3, name: 'Leslie Raul', url: '#' },
    // Más personas...
]

// Recibe las propiedades y define los eventos que este componente puede emitir
defineProps(['open'])
const emit = defineEmits(['close'])

// Aquí tienes tu lógica reactiva y computada
const query = ref('')
const filteredPeople = computed(() => query.value === ''
    ? []
    : people.filter((person) => person.name.toLowerCase().includes(query.value.toLowerCase())))

// Función que se llama al seleccionar una persona. Por ejemplo, podría redirigir al usuario.
function onSelect(person) {
    window.location = person.url
}

// Aquí se cierra el componente y se emite el evento 'close'
function closeCommand() {
    query.value = ''
    emit('close') // Aquí usas emit para disparar el evento 'close'
}
</script>